...
select "akka://MyActorSystem/user/AuthenticationActor" mailbox.Context.System <! username
```
...
A very commonly used `IActorRef` is `Sender`, which is the `IActorRef` made available to every actor context and is a handle to the sender of the message currently being processed. In an earlier lesson, we used this inside `fileValidatorActor`'s message receiver to easily send a confirmation message back to the sender of the message that was being validated by `fileValidatorActor`.
...
Very simple: `let selection = select "/path/to/actorName" mailbox.Context.System`

> NOTE: **the path to an actor includes the name you assign to an actor when you instantiate it, NOT its class name**. For example:

```fsharp
// the ActorPath for myFooActor is "/user/barBazActor"
// NOT "/user/myFooActor" or "/user/fooActor"
let myFooActor = spawn myActorSystem "barBazActor" (actorOf fooActor)
```

### Do I send a message differently to an `ActorSelection` vs an `IActorRef`?
Nope. You `<!` (aka tell) an `ActorSelection` a message just the same as an `IActorRef`:

```fsharp
let selection = select "/path/to/actorName" mailbox.Context.System
selection <! message
```
...
BUT, imagine that `consoleReaderActor` was far away in the hierarchy from where the `fileValidatorActor` instance was created (`Program.fs` currently). In this case, there is no clean/easy way to pass in the needed `IActorRef` to `consoleReaderActor` without also passing it through every intermediary first.
...
Let's fix that by **removing the `validationActor` `IActorRef` that we're passing in**. The declaration of `consoleReaderActor` should now look like this:
...
Finally, let's update `consoleReader` accordingly in `Program.fs` since it no longer takes in the `validationActor`:
```fsharp
...
First, **remove the `tailCoordinatorActor` argument to the constructor of `fileValidatorActor` and remove the accompanying field on the class**. The `fileValidatorActor` should now look like this:
